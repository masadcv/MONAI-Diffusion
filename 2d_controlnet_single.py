...
train_transforms_sl = transforms.Compose(
    [
        ...
        transforms.EnsureChannelFirstd(keys=["source", "target"], channel_dim=-1),
        # transforms.EnsureTyped(keys=["source", "target"]),
        # transforms.RandSpatialCropd(keys=["source", "target"], roi_size=(64, 64), random_size=False),
        transforms.Resized(
            keys=["source", "target"],
            spatial_size=(64, 64),
...
source_images_path = glob.glob(os.path.join(source_folder_path, "*"))
target_images_path = [
    os.path.join(target_folder_path, os.path.basename(p)) for p in source_images_path
]

for image in source_images_path:
...
list_dict = [
    {"source": s, "target": t} for s, t in zip(source_images_path, target_images_path)
]
...
for epoch in range(n_epochs):
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        Image.fromarray(source[sel_batch]).save(os.path.join(output_dir, "cn_training_source_{}.png".format(epoch)))
...
for k in range(num_samples):
    Image.fromarray(val_source[k]).save(os.path.join(output_dir, "cn_val_source_{}.png".format(k)))
